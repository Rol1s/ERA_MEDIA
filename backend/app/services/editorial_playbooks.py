...
PLAYBOOKS: dict[str, dict[str, Any]] = {
    "era-now": {
        "purpose": "Write current events as a living editorial voice: explain the signal, context, and consequences without panic, clickbait, or summary scaffolding.",
        "audience": "Readers who want a smart human view on the day, not an AI brief.",
        "editorial_formula": ["strong editorial opening", "source-backed context", "human interpretation", "consequence or tension"],
        "tone_of_voice": "Calm, observant, human, slightly opinionated, source-aware.",
        "content_pillars": ["events", "public impact", "practical context", "editorial angle", "what tension is forming"],
        ...
        "good_post_examples": ["Это не просто очередное решение регулятора. Видно, как платформы постепенно перестраивают интернет под новую норму контроля."],
        ...
        "required_structure": ["editorial opening", "source-backed context", "interpretation", "consequence"],
        "banned_patterns": ["шок", "срочно без источника", "точно будет", "все пропало", "что произошло", "почему это важно", "что дальше", "в чём суть"],
        "cta_style": "Soft watch-next CTA.",
        "max_post_length": 1100,
        "min_editorial_value_score": 72,
    },
    "era-money": {
        "purpose": "Turn business and money facts into a sharp non-advisory market read, like a journalist who has been watching the pattern for months.",
        "audience": "Entrepreneurs, operators, investors, and financially curious readers.",
        "editorial_formula": ["market signal", "money mechanic", "hidden tension", "practical implication"],
        "tone_of_voice": "Practical, sober, human, no financial advice, no template headings.",
        ...
        "good_post_examples": ["Рынок снова показал, что рост на плечах выглядит уверенно только до первой проверки ликвидности."],
        ...
        "required_structure": ["market signal", "money mechanic", "risk/tension", "reader implication"],
        "banned_patterns": ["гарантированный доход", "сигнал на покупку", "точно заработаете", "где здесь деньги", "риски", "кому полезно", "что проверить", "идея / факт"],
        "cta_style": "Ask reader to verify one number or assumption.",
        "max_post_length": 1300,
        "min_editorial_value_score": 76,
    },
    "era-ai": {
        "purpose": "Discuss AI and automation as an active observer with a practical worldview: what changes in work, infrastructure, power, and habits.",
        "audience": "Builders, creators, founders, operators, and AI-curious professionals.",
        "editorial_formula": ["sharp observation", "plain mechanism", "operator consequence", "limit or test"],
        "tone_of_voice": "Clear, hands-on, curious, realistic, human, no AI-summary scaffolding.",
        ...
        "good_post_examples": ["Здесь важна не сама демка, а то, как ИИ quietly переезжает из отдельного приложения в рабочий слой, где человек уже принимает решения."],
        ...
        "required_structure": ["observation", "mechanism", "operator implication", "limit/test"],
        "banned_patterns": ["революция без деталей", "заменит всех", "100% автономно", "что изменилось", "как работает", "как применить", "кому полезно", "попробовать сегодня"],
...
def playbook_summary(channel_slug: str) -> str:
    ...
    return "\n".join(
        [
            ...
            f"Formula (internal only, do not print as headings): {' / '.join(playbook['editorial_formula'])}",
            f"Tone: {playbook['tone_of_voice']}",
            f"Required structure (invisible narrative ingredients, not visible sections): {' / '.join(playbook['required_structure'])}",
